...
function renderCmt() {
  const html = data.comment.map((cmt)=> `
    <div class="cmt">
      <div class="cmt__user">
          ${(cmt.userName==userName||userName=='admin')?'<button class="rmCmt" onclick="removeCmt(this)">x</button>' :''}
...
function removeCmt(e) {
  ...
  const us = e.parentElement.querySelector('.cmt__user_name').textContent
